...
@synthesize m_buyCount;
@synthesize m_specInfo;
...
-(id)init
{
    ...
    if (self) {
        ...
        m_productName  = @"";
        m_productCost  = @"";
        m_productImgUrl  = @"http://images.apple.com/v/watch/g/images/shared/og_home.jpg?201511200804";
        m_productPrice  = @"";
        m_buyCount = @"";
        m_saleCount = @"0";
        m_detailUrl = @"http://www.apple.com/tv/";
...
{
    if ([self init]) {
        if (![NNCommonDeal jugementArrIsNil:[dic valueForKeyPath:@"specList"]]) {
            self.m_sizeListArr = [dic valueForKeyPath:@"specList"];
        }
        ...
        if ([NNCommonDeal jugementStringIsNil:[dic valueForKeyPath:@"buyCount"]]) {
            self.m_buyCount = @"";
        }else{
            self.m_buyCount = [NSString stringWithFormat:@"%@",[dic valueForKeyPath:@"buyCount"]];
        }
        
        if ([NNCommonDeal jugementDicIsNil:[dic valueForKeyPath:@"buySpecInfo"]]) {
            self.m_specInfo = nil;
        }else{
            self.m_specInfo = [dic valueForKeyPath:@"buySpecInfo"];
        }
