...
-(void)layoutShopCartView
{
    ...
    MJRefreshStateHeader *header = [MJRefreshStateHeader headerWithRefreshingBlock:^{
        if (m_nnShopCartDelegate && [m_nnShopCartDelegate respondsToSelector:@selector(reloadShoppingCartData:andFailed:)]) {
            [m_nnShopCartDelegate reloadShoppingCartData:^(id sucBack) {
                NSMutableArray *arr = (NSMutableArray *)sucBack;
                [self refrshShoppingCartData:arr andLike:nil];
                [m_shopCartTableView.header endRefreshing];
    ...
    m_shopCartTableView.header = header;
...
-(IBAction)selectAllBtnClick:(id)sender
{
    if ([AppManage isLogin]) {
        if (!m_cartListArray) {
            [SVProgressHUD showInfoWithStatus:@"购物车还是空的哦" maskType:SVProgressHUDMaskTypeClear];
            return;
        }
...
-(IBAction)payBtnClick:(id)sender
{
    if ([AppManage isLogin]) {
        ...
        if (m_selectListArray.count == 0) {
            [SVProgressHUD showInfoWithStatus:@"您还没有选择商品" maskType:SVProgressHUDMaskTypeClear];
            return;
        }
        //1.确认选中的商品，获取单价和数量，计算总价
...
-(HJCAjustNumButton *)addAjustButton:(ProductModel *)productM andTag:(NSInteger)tag
{
    ...
    btn.frame = CGRectMake(0, 0, 102 , 34);
    btn.lineColor = [UIColor whiteColor];
    btn.tag = tag;
...
-(void)calculatePrice
{
    ...
    //判断是否全选
    if (m_selectListArray.count == m_cartListArray.count && m_selectListArray.count>0) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.p_imageBtn.imageView.contentMode = UIViewContentModeScaleToFill;
    [cell.p_imageBtn sd_setImageWithURL:[NSURL URLWithString:productM.m_productImgUrl] forState:UIControlStateNormal placeholderImage:[AppManage getDefaultImage:@"80"]];
    cell.p_imageBtn.tag = indexPath.section;
    [cell.p_imageBtn addTarget:self action:@selector(imageBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    cell.p_nameTextView.attributedText = [self getProductText:productM.m_productName andSizeText:productM.m_specInfo];
    cell.p_priceLbl.text = [NSString stringWithFormat:@"¥%@",productM.m_productCost];
...
-(void)imageBtnClick:(id)sender
{
    ...
    ProductModel *productM = [m_cartListArray objectAtIndex:imgBtn.tag];
