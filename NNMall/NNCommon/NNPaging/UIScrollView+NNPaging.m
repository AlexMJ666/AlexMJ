...
#import <MJRefresh.h>
#import "UIScrollView+NNPaging.h"

static float kAnimationDuration = 0.2f;

static const char nn_originContentHeight;
static const char nn_secondScrollView;
...
- (void)setOriginContentHeight:(float)originContentHeight {
    objc_setAssociatedObject(self, &nn_originContentHeight, @(originContentHeight), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (float)originContentHeight {
    return [objc_getAssociatedObject(self, &nn_originContentHeight) floatValue];
}
...
- (UIScrollView *)secondScrollView {
    return objc_getAssociatedObject(self, &nn_secondScrollView);
}

- (void)setSecondScrollView:(UIScrollView *)secondScrollView {
    objc_setAssociatedObject(self, &nn_secondScrollView, secondScrollView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
