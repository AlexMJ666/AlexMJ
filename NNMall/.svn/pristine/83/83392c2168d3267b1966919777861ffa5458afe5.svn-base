...
@interface NNProductInfoView ()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,SDCycleScrollViewDelegate,NNProCommentDelegate,SDPhotoBrowserDelegate>
{
    UIScrollView        *m_productScrollView;
    UITableView         *m_productTableView;
    UIWebView           *m_productWebView;
    SDCycleScrollView   *m_cycleView;
    SDPhotoBrowser      *m_browser;
...
@property (nonatomic,strong) SDPhotoBrowser         *p_browser;
...
@synthesize p_browser = m_browser;
-(void)dealloc
{
...
-(void)initPhotoBrowser:(NSInteger)count withIndex:(NSInteger)index
{
    SDPhotoBrowser *browser = [[SDPhotoBrowser alloc] init];
    browser.sourceImagesContainerView = m_cycleView;
...
{
    return [UIImage imageNamed:@"back414-166.png"];
}

// 返回高质量图片的url
- (NSURL *)photoBrowser:(SDPhotoBrowser *)browser highQualityImageURLForIndex:(NSInteger)index
{
    return [NSURL URLWithString:self.p_productModel.m_productImgUrl];
}
...
{
    [self initPhotoBrowser:3 withIndex:index];
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //计算高度 默认：132
    CGSize contentSize = [m_commentModel.m_content boundingRectWithSize:CGSizeMake(kMainScreen_Width-30, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]} context:nil].size;
    if (contentSize.height>20) {
        return contentSize.height+111;
    }
    return 132.0f;
}
