...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demo:(NSString* )billID
                              andPrice:(NSString*)price
                               andType:(NSString*)type
                             andAttach:(NSString *)parkName
{

    //订单标题，展示给用户
    NSString *order_name    = type;
    //订单金额,单位（分）
...
//    NSLog(@"%lu",(unsigned long)noncestr.length);
    NSMutableDictionary *packageParams = [NSMutableDictionary dictionary];
    
    [packageParams setObject: appid             forKey:@"appid"];           //开放平台appid
    [packageParams setObject: mchid             forKey:@"mch_id"];          //商户号
//    [packageParams setObject: parkName          forKey:@"detail"];          //标题-停车场
//    [packageParams setObject: @"APP-001"        forKey:@"device_info"];   //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];       //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];      //支付类型，固定为APP
    [packageParams setObject: [NSString stringWithFormat:@"%@-%@",parkName,order_name]        forKey:@"body"];            //订单描述，展示给用户
//    http://192.168.1.31:8080/smart-park-server//api/v2/wx/pay/callback
    [packageParams setObject: [NSString stringWithFormat:@"%@%@",NNHttpServiceUrl,NOTIFY_URL]        forKey:@"notify_url"];      //支付结果异步通知
    [packageParams setObject: billID            forKey:@"out_trade_no"];    //商户订单号
//    [packageParams setObject: billID            forKey:@"product_id"];      //商品id
