...
@property (nonatomic,strong) UILabel     *p_proStatusLbl;
...
@synthesize p_proStatusLbl = m_proStatusLbl;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.p_proStatusLbl = [[UILabel alloc]init];
        m_proStatusLbl.font = [NNCommonDeal getFont:12];
        m_proStatusLbl.textAlignment = NSTextAlignmentRight;
        [self.contentView addSubview:m_proStatusLbl];
        
        self.p_serviceIco = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@""]];
        [self.contentView addSubview:m_serviceIco];
        self.p_serviceLbl = [[UILabel alloc]init];
        m_serviceLbl.font = [NNCommonDeal getFont:12];
        m_serviceLbl.text = @"由暖暖商城发货并提供售后服务";
        [self.contentView addSubview:m_serviceLbl];
        
        self.p_propmtIco = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@""]];
        [self.contentView addSubview:m_propmtIco];
        self.p_propmtLbl = [[UILabel alloc]init];
        m_propmtLbl.font = [NNCommonDeal getFont:12];
        m_propmtLbl.text = @"支持7天无理由退款";
...
-(void)createProductCellView:(ProductModel *)productM andNameHeight:(CGFloat)height
{
    ...
    m_proPriceLbl.frame = CGRectMake(m_proNameLbl.frame.origin.x, m_proNameLbl.frame.origin.y+newHeight+15, kMainScreen_Width-130, 42);
    m_proStatusLbl.frame = CGRectMake(kMainScreen_Width-115, m_proPriceLbl.frame.origin.y+15, 100, 21);
    m_serviceIco.frame = CGRectMake(m_proNameLbl.frame.origin.x, m_proPriceLbl.frame.origin.y+m_proPriceLbl.frame.size.height+24, 48, 18);
    m_serviceLbl.frame = CGRectMake(m_serviceIco.frame.origin.x+m_serviceIco.frame.size.width+15, m_serviceIco.frame.origin.y, 200, 18);
    
    m_propmtIco.frame = CGRectMake(m_proNameLbl.frame.origin.x, m_serviceIco.frame.origin.y+m_serviceIco.frame.size.height+4, 48, 18);
    m_propmtLbl.frame = CGRectMake(m_propmtIco.frame.origin.x+m_propmtIco.frame.size.width+15, m_propmtIco.frame.origin.y, 200, 18);
    ...
    m_proStatusLbl.text = [NSString stringWithFormat:@"已有%@人购买",productM.m_saleCount];
